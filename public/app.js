const $=s=>document.querySelector(s); let cart=[]; let catalog=[];
const money=n=>new Intl.NumberFormat('ru-RU',{style:'currency',currency:'RUB',maximumFractionDigits:0}).format(n);
async function api(url,data){const r=await fetch(url,data?{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}:{});const value=await r.json();if(!r.ok)throw Error(value.error);return value;}
let timer;function status(text){$('#status').textContent=text;clearTimeout(timer);timer=setTimeout(()=>$('#status').textContent='',6000);}
function handle(fn){return async event=>{event?.preventDefault();try{await fn(event);}catch(e){status(e.message);}};}
...
async function productsRefresh(){await products($('#query').value);await admin();}
$('#refreshAdmin').onclick=handle(admin);$('#orderFilter').onchange=handle(admin);
$('#register').onsubmit=handle(async()=>{await api('/api/register',{name:$('#regName').value,email:$('#regEmail').value,password:$('#regPassword').value});$('#email').value=$('#regEmail').value;$('#password').value=$('#regPassword').value;status('Аккаунт создан. Войдите в личном кабинете.');$('#account').scrollIntoView({behavior:'smooth'});});
$('#search').onsubmit=handle(()=>products($('#query').value));
$('#login').onsubmit=handle(async()=>{await api('/api/login',{email:$('#email').value,password:$('#password').value});await account();status('Вы вошли');});
$('#logout').onclick=handle(async()=>{await api('/api/logout');await account();});
