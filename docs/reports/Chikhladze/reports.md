...
**Автор:** Chikhladze · **Проект:** Faultline Shop · **Раздел:** 9 учебных сценариев работы с файлами и сетью
...
| [FILE-03 · Перезапись файла без владельца](#file-03) | `POST /api/upload` → `name`, `content` | Содержимое существующего файла | Повторная анонимная загрузка `note.txt` заменяет `first` на `second` |
| [FILE-04 · Перечисление загруженных файлов](#file-04) | `GET /api/uploads` | Доступность списка имён без входа | Имя `class-a-note.txt` в ответе после выхода из аккаунта |
| [FILE-05 · Публичная резервная копия](#file-05) | `GET /backup/users.json` | Доступность данных пользователей без входа | Учебные пользователи и пароли в ответе после выхода из аккаунта |
| [FILE-06 · Нет общей квоты хранения](#file-06) | `POST /api/upload`; `GET /api/uploads` | Число файлов и суммарный объём хранения | Три небольших файла сохраняются; в обработчике отсутствует учёт общей квоты |
| [FILE-07 · Бессрочное хранение и публичные URL](#file-07) | `POST /api/upload`; `GET /uploads/retention.txt` | Доступность файла после завершения сессии | `retention.txt` остаётся доступным после выхода из аккаунта |
| [NET-01 · SSRF во внутренний сервис](#net-01) | `GET /api/import` → `url` | Сервер запрашивает внутренний mock-сервис по переданному URL | Учебный ключ `LAB_ONLY_INTERNAL_KEY_42` в ответе импорта |
| [NET-02 · HTTP без TLS](#net-02) | `http://127.0.0.1:3000`, `http.createServer` | Транспорт не шифрует запросы | HTTP в URL и конфигурации сервера; удалённый перехват этим не подтверждается |
...
<a id="file-06"></a>
# FILE-06: Нет общей квоты хранения — средняя

## Воспроизведение, проявление и лечение

**Механизм:** один JSON ограничен примерно 100 КБ, но число файлов и общий размер не ограничены. **Безопасная проверка:** загрузите три маленьких файла с разными именами и посмотрите `/api/uploads`; все остаются. Не запускайте циклы, заполняющие диск.

**Влияние:** исчерпание диска в реальном сервисе. **Где:** `/api/upload`; отсутствует учёт общего объёма. **Лечение:** квоты на пользователя и весь сервис, лимит числа объектов, мониторинг, атомарный отказ до записи. Проверка после исправления использует малый тестовый лимит и уборку fixtures.

<a id="file-07"></a>
# FILE-07: Бессрочное хранение и публичные URL — средняя

## Воспроизведение, проявление и лечение

**Механизм:** загруженный файл не имеет owner, срока жизни или операции удаления; URL остаётся доступным после logout. **Проверка:** загрузите `retention.txt`, выйдите, откройте `/uploads/retention.txt`.

**Влияние:** данные переживают ожидаемый жизненный цикл сессии и доступны всем, кто знает имя. **Лечение:** политика retention, owner metadata, авторизация чтения, управляемое удаление и фоновые задачи очистки. Это отличается от FILE-03: даже без перезаписи первый файл остаётся публичным навсегда.

<a id="net-01"></a>
# NET-01: SSRF во внутренний сервис — высокая; ограниченная демонстрация

## Воспроизведение, проявление и лечение

**Механизм:** backend получает URL и читает ресурс, недоступный как обычный маршрут магазина. Пользователю передаётся полный ответ внутреннего сервиса. **Где:** `/api/import` и отдельный HTTP-сервис warehouse на 3001.

**Проверка:** `await lab('/api/import?url='+encodeURIComponent('http://127.0.0.1:3001/secret'))` возвращает `LAB_ONLY_INTERNAL_KEY_42`. Запрос выполняет сервер, а не браузер. В учебной модели warehouse играет роль внутренней системы, хотя на этой машине loopback доступен и самому пользователю.

**Влияние:** раскрытие данных внутренней службы. Ограничитель разрешает только mock-origin, блокирует редиректы; произвольное сканирование сети и cloud metadata не реализованы.

**Лечение:** не принимать произвольный URL; принимать ID разрешённой операции, формировать адрес на сервере, авторизовать запрос к warehouse, ограничить поля ответа. Для реальных URL-import: строгий allowlist, сетевой egress-контроль, защита от DNS rebinding и повторная проверка редиректов. После исправления секрет не выдаётся обычному пользователю.

<a id="net-02"></a>
# NET-02: HTTP без TLS — средняя; свойство локального развёртывания

## Воспроизведение, проявление и лечение

**Механизм:** http-сервер не шифрует запросы. **Признак:** адрес начинается `http://`, в DevTools видны поля учебного входа. Это не доказательство, что посторонний узел может перехватить loopback-трафик.

**Где:** `http.createServer`, запуск на `127.0.0.1`. **Влияние:** при неверном переносе в общую сеть пароли и cookie не защищены транспортом. **Проверка:** посмотрите URL и конфигурацию сервера; сетевой перехват не требуется.

**Лечение для реального сервиса:** HTTPS с корректным сертификатом, редирект HTTP→HTTPS, Secure-cookie, затем HSTS с обдуманным охватом доменов. В этой версии HTTP выбран для локальной лаборатории; не считайте отсутствие TLS доказательством удалённой эксплуатации.
